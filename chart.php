...
if(stripos($ua,'Android 2.') !== false)
	...
	*///typer som ikke kan bruke svg i leseren..
	if($svg===false)
		$imagechart=true;
	else
		$imagechart=false;
...
?>
	...
	function chart($imagechart=false,$divname,$data,$tittel,$top='6.7',$bunn='3')
	{
      ...
      function drawChart() {
		...
		foreach($data as $a)
		{
			...
			$type=($a['kort']==0)?'Langtidsvirkende':'Korttidsvirkende';
			$a['insulin']=($a['kort']==0)?$a['insulin']*-1:$a['insulin'];
		...
		if($imagechart===true)
		{
			...
			echo "var options = {};  options.colors = ['#FF0000' , '#00FF00']; options.fill = true;options.min = 0;
			var chart = new google.visualization.ImageAreaChart(document.getElementById('chart_$divname')).draw(data, options);";
		}else
		{
		echo " var formatter = new google.visualization.NumberFormat(
  ...
  formatter.format(data, 1);";
    ...
    <script type="text/javascript">
      function drawVisualization() {
        var dashboard = new google.visualization.Dashboard(
		 ...
		 data.addColumn({type:'string', role:'tooltip', p: {html: true}});
		 data.addColumn('number', 'Top (<?php echo $top; ?>)');
		 data.addColumn('number', 'Bunn (<?php echo $bunn; ?>)');
      <?php
	  foreach($avgset as $u)
	  {
			echo 'var date = new Date('.$u['dag'].');';
			echo 'data.addRow([date, '.$avg[0][0].','.$u['avg'].",createtooltip('".$u['dato']."','".$u['avg']."')".','.$top.','.$bunn.']);';
	  }
	  ?>
	...
	function createtooltip(dato,verdi)
	{
		return '<p>'+
		'<table>'+
		'<tr>'+'<td>'+'Dato:'+'</td>'+'<td><strong>'+dato+'</strong></td>'+'</tr>'+
		'<tr>'+'<td>'+'Verdi:'+'</td>'+'<td><strong>'+verdi+'</strong></td>'+'</tr>'+
